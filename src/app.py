...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    while step < MAX_ITERATIONS:
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            # Đưa Observation trở lại LLM để nó quyết định gọi Tool tiếp theo
            # hoặc tổng hợp câu trả lời cuối cùng.
            agent_context = (
                f"Yêu cầu ban đầu: {user_query}\n"
                f"Tool vừa thực thi: {tool_name}\n"
                f"Observation gần nhất (JSON): {obs_str}\n"
                "Dựa trên Observation, hãy tiếp tục mục tiêu ban đầu. "
                "Không gọi lại Tool vừa thành công. Nếu cần thêm hành động, "
                "hãy gọi Tool tiếp theo; nếu đã đủ thông tin, hãy trả lời kết quả cuối cùng."
            )

        else:
            final_content = "LLM trả về kiểu phản hồi không được hỗ trợ."
            print(f"🏁 [Final Answer]: {final_content}")
            trace_logs.append({
                "step": step,
                "query": user_query,
        ...
        final_content = "Không thể hoàn tất yêu cầu trong giới hạn số vòng lặp cho phép."
        print(f"🏁 [Final Answer]: {final_content}")
        trace_logs.append({
